...
describe('add tool', () => {
  ...
  describe('handler', () => {
    ...
    it('adds decimal numbers', async () => {
      ...
      expect((result.content[0]! as { text: string }).text).toBe('4');
      expect(result.isError).toBe(false);
    });

    it('handles zero', async () => {
      const result = await handler({ a: 0, b: 0 });
      expect((result.content[0]! as { text: string }).text).toBe('0');
      expect(result.isError).toBe(false);
    });

    it('coerces string numbers', async () => {
      const result = await handler({ a: '10' as unknown as number, b: '20' as unknown as number });
      expect((result.content[0]! as { text: string }).text).toBe('30');
      expect(result.isError).toBe(false);
    });
