...
describe("get_weather tool", () => {
  ...
  describe("handler", () => {
    ...
    it("throws when current_weather is missing", async () => {
      ...
      await expect(
        handler({ latitude: 48.8566, longitude: 2.3522 })
    ...
    it("throws on invalid latitude (out of range)", async () => {
      await expect(
        handler({ latitude: 100, longitude: 2.3522 })
      ).rejects.toThrow();
    });

    it("throws on invalid longitude (out of range)", async () => {
      await expect(
        handler({ latitude: 48.8566, longitude: 200 })
      ).rejects.toThrow();
    });

    it("throws on non-numeric latitude", async () => {
      await expect(
        handler({ latitude: "not-a-number", longitude: 2.3522 })
      ).rejects.toThrow();
    });
